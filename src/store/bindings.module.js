...
export default {
  [BIND_SUBJECTS]: firestoreAction(async ({ bindFirestoreRef, rootState }) => {
    if (rootState.user.isTeacher)
      ...
      let groupsWithStudent = await db.collection('groups')
        .where('students', 'array-contains',
          db.collection('users').doc(rootState.user.uid)).get()
  ...
  [BIND_GROUPS]: firestoreAction(({ bindFirestoreRef, rootState }, subjectId) => {
    if (rootState.user.isTeacher)
  ...
  [BIND_TASKS]: firestoreAction(({ bindFirestoreRef, rootState }, subjectId) => {
    if (rootState.user.isTeacher)
  ...
  [BIND_MARKS]: firestoreAction(({ bindFirestoreRef }, subjectId) => {
    return bindFirestoreRef('marks', db.collection('marks')
      .where('subject', '==', db.collection('subjects').doc(subjectId)))
  })
}
